...
class Pacman {
	...
	class GamePanel extends JPanel implements KeyListener {
		...
		public void drawGrid(Graphics g) {
			...
			if (isWinner) {
				...
				g.setColor(Color.blue);
				g.setFont(endFont);
				g.drawString("YOU WIN!" , 95, 250);
			}
			if (isDead) {
				stopTimers();
				g.setColor(Color.red);
				g.setFont(endFont);
				g.drawString("YOU LOSE!", 80, 250);	// Draw at end so a white square doesn't overlap it
			}
		}
	...
	class MonsterLoop implements ActionListener {
		byte[][] oldCoords, newCoords;	// Need to save coords to update after generating new locations
		
		// Move monster in the cardinal directions
		public void actionPerformed(ActionEvent e) {
			// oldCoords = new int[6][2];
			newCoords = new byte[6][2];
			byte monstersFound = 0;
			
			for (byte row = 0; row < 10; row++)
				for (byte col = 0; col < 10; col++) {
					// if there are monsters
					if (board[row][col][1] > 0)
						for (byte mon = 0; mon < board[row][col][1]; mon++) {
							//oldCoords[monstersFound][0] = row;
							//oldCoords[monstersFound][0] = col;
							...
							newrow %= 10;
							newcol %= 10;
							if (newrow == -1) newrow = 9;
							if (newcol == -1) newcol = 9;
							// remove monster from board
							board[row][col][1]--;
							// add new monster to board
							// board[newrow][newcol][1]++;
							System.out.println(monstersFound);
							if (monstersFound < 6) {
								newCoords[monstersFound][0] = newrow;
								newCoords[monstersFound][1] = newcol;
							}
							monstersFound++;
						}
				}
			for (byte[] xy: newCoords)
				board[xy[0]][xy[1]][1]++;
			panel.repaint();
		}
	}
