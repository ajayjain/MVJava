...
// January 14, 2013
// Mars.java
// This class is a mini RPG terminal game set on MARS where you control
// a rover searching for ice, hematite and aliens!
...
public class Mars {
	...
	private void GameOver() {
		System.out.println("\n\nGAME OVER!");
		System.out.println("Final Computed Score: "+(icecount*2+hemcount+(foundalien ? 10 : 0)));
		System.out.printf("COLLECTED:  %d ICE SAMPLES,\t%d HEMATITE SAMPLES %8d ALIEN ENCOUNTERS\n\n", icecount, hemcount, foundalien ? 1 : 0);
	}
	...
	private void PrintPlanet() {
		...
		for (int r = 0; r < n; r++) {
			...
			// Print row
			for (int c = 0; c < n; c++) {
				if ((Math.abs(r-rover[0])+Math.abs(c-rover[1])) <= 3) // Field of view -> max distance = 3
